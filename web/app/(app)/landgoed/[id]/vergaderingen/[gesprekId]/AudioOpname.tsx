...
type Status = "idle" | "opnemen" | "verwerken" | "klaar" | "fout";
    ...
    <div className="flex flex-col gap-3">
      ...
      <div>
        ...
        {status === "idle" || status === "fout" || status === "klaar" ? (
          ...
          <div className="flex items-center gap-3">
            <span
              ...
              style={{ background: "var(--red)", animation: "pulse 1s infinite" }}
            />
            <span className="font-mono text-[14px]">{fmt(seconden)}</span>
            <button type="button" onClick={stopOpname} className="btn btn-ghost btn-sm">
        ...
        {!transcriptiesBeschikbaar && (
          <p className="mt-1 text-[12px]" style={{ color: "var(--text-3)" }}>
            GROQ_API_KEY ontbreekt — voeg deze toe in Vercel.
          </p>
        )}
      </div>
      ...
      <div>
        ...
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            const input = (e.currentTarget.elements.namedItem("audio") as HTMLInputElement);
        ...
        >
          <input
            ...
            disabled={!transcriptiesBeschikbaar || status === "verwerken"}
          />
          <button
            type="submit"
            ...
            disabled={!transcriptiesBeschikbaar || status === "verwerken"}
          >
            {status === "verwerken" ? "Bezig…" : "Transcribeer"}
          </button>
        </form>
      </div>
      ...
      {/* Resultaat / fout */}
      {fout && (
        <div className="text-[13px]" style={{ color: "var(--red)" }}>{fout}</div>
      )}
      {status === "klaar" && transcript && (
        <div className="rounded p-3 text-[12.5px]" style={{ background: "var(--bg-2)", color: "var(--text-2)" }}>
          ...
          <div className="mt-1 text-[11.5px]" style={{ color: "var(--text-3)" }}>
            Scroll omhoog om het volledige transcript te zien en bewerkingen te starten.
          </div>
        </div>
      )}
    </div>
